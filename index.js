...
function isPrimeNumber(num) { 
  if (num<=1) {return 'false'} else {for(let i=2; i<num; i++){
...
function isPositiveNumber(num) {
  ...
  if(num>0){return 'true'} else if(num<=0){return 'false'}
...
function checkDataType(value) {
  ...
  let x = typeof value 
  console.log(`Значення ${value} має тип ${x}`)
...
function findMax(num1, num2) {
  ...
  if (num1>num2){return num1} else if(num2>=num1){return num2}
...
function getUserStatus(age) {
  ...
  if(age<18){return 'Неповнолітній'} else if (age>=18, age<65){return 'Дорослий'} 
  else (age>=65);{return 'Пенсіонер'}
...
function getVariableType(variable) {
  ...
  if(x==='number'){return 'це число'}else if(x==='string'){return 'це рядок'}
  else if(x==='boolean'){return 'Булеве значення'}else {return 'це інший тип'}
...
function divide(x){return function(y){return x/y} }
...
function memoizedArea() {
  ...
  return function(side){if (side===prevSide) {console.log("Fetching from cache"); return prevResult;} 
  else{console.log("Calculating result");
     let result = side*side; prevSide=side; prevResult=result;
    return result;}
  };
...
function compose(increment, double) {
  ...
  return  function incrementAnddouble(n){return double(increment(n));}
